...
#if defined(NUM_RUNOUT_SENSORS  )
  const int num_runout_sensors   = NUM_RUNOUT_SENSORS  ; //  2
#endif
#if defined(FIL_RUNOUT_INVERTING)
...
const char *filament_runout_script = "M600";
  ...
  const int manual_y_home_pos = MANUAL_Y_HOME_POS;
#endif
...
#endif
#if defined(Y_DUAL_ENDSTOPS_ADJUSTMENT )
...
#endif
#if defined(BABYSTEP_MULTIPLICATOR)
...
#endif
#if defined(Y2_DIR_PIN)
  const int y2_dir_pin = Y2_DIR_PIN;
#endif
#if defined(Y2_ENABLE_PIN)
...
#if defined(DOGLCD_A0)
  const int doglcd_a0 = DOGLCD_A0;
#endif
#if defined(DOGLCD_CS)
...
#if defined(STAT_LED_RED_PIN)
  const int stat_led_red_pin = STAT_LED_RED_PIN;
#endif
#if defined(STAT_LED_BLUE_PIN)
